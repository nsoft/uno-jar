...
// Changes from original ConsoleAppender : Rename class, reflection for ConsoleFactory.getManager()
...
import java.util.concurrent.atomic.AtomicInteger;
   ...
   * </p>
   */
  @Plugin(name = TestConsoleAppender.PLUGIN_NAME, category = Core.CATEGORY_NAME, elementType = Appender.ELEMENT_TYPE, printObject = true)
  public final class TestConsoleAppender extends AbstractOutputStreamAppender<OutputStreamManager> {

    public static final String PLUGIN_NAME = "TestConsole";
    ...
    private TestConsoleAppender(final String name, final Layout<? extends Serializable> layout, final Filter filter,
     ...
     * @return The TestConsoleAppender.
     * @deprecated Deprecated in 2.7; use {@link #newBuilder()}.
     */
    @Deprecated
    public static TestConsoleAppender createAppender(Layout<? extends Serializable> layout,
                                                 final Filter filter,
                                                 ...
                                                 final String ignore) {
      if (name == null) {
        AbstractLifeCycle.LOGGER.error("No name provided for TestConsoleAppender");
        return null;
      }
      if (layout == null) {
      ...
      return new TestConsoleAppender(name, layout, filter, getManager(target, isFollow, false, layout), ignoreExceptions, target);
    }
     ...
     */
    @Deprecated
    public static TestConsoleAppender createAppender(
        // @formatter:off
      ...
      if (follow && direct) {
        AbstractLifeCycle.LOGGER.error("Cannot use both follow and direct on TestConsoleAppender");
        return null;
      }
      return new TestConsoleAppender(name, layout, filter, getManager(target, follow, direct, layout), ignoreExceptions, target);
    }

    public static TestConsoleAppender createDefaultAppenderForLayout(final Layout<? extends Serializable> layout) {
      // this method cannot use the builder class without introducing an infinite loop due to DefaultConfiguration
      return new TestConsoleAppender("DefaultConsole-" + COUNT.incrementAndGet(), layout, null,
    ...
    /**
     * Builds TestConsoleAppender instances.
     * @param <B> The type to build
     */
    public static class Builder<B extends Builder<B>> extends AbstractOutputStreamAppender.Builder<B>
        implements org.apache.logging.log4j.core.util.Builder<TestConsoleAppender> {
      ...
      @Override
      public TestConsoleAppender build() {
        if (follow && direct) {
          throw new IllegalArgumentException("Cannot use both follow and direct on TestConsoleAppender '" + getName() + "'");
        }
        final Layout<? extends Serializable> layout = getOrCreateLayout(target.getDefaultCharset());
        return new TestConsoleAppender(getName(), layout, getFilter(), getManager(target, follow, direct, layout),
    ...
    private static OutputStream getOutputStream(final boolean follow, final boolean direct, final Target target) {
      ...
      try {
        ...
        AbstractLifeCycle.LOGGER.debug("Jansi is not installed, cannot find {}", JANSI_CLASS);
      } catch (final NoSuchMethodException nsme) {
        AbstractLifeCycle.LOGGER.warn("{} is missing the proper constructor", JANSI_CLASS);
      } catch (final Exception ex) {
        AbstractLifeCycle.LOGGER.warn("Unable to instantiate {} due to {}", JANSI_CLASS, clean(Throwables.getRootCause(ex).toString()).trim());
      }
      return outputStream;
    }
    ...
    private static class ConsoleManagerFactory implements ManagerFactory<OutputStreamManager, FactoryData> {
      ...
      public OutputStreamManager createManager(final String name, final FactoryData data) {
        try {
          Constructor<OutputStreamManager> cons = OutputStreamManager.class.getDeclaredConstructor(OutputStream.class, String.class, Layout.class, Boolean.TYPE);
          cons.setAccessible(true);
          return cons.newInstance(data.os, data.name, data.layout, true);
